...
function [Vx_f,Vz_f,theta_f,q_f,Vel_f,gamma_f,Vx_b,Vz_b,theta_b,q_b,Vel_b,gamma_b,u1_f,u2_f] = reachability()
    ...
    for k = 1:N-1
        ...
        [k_11,k_12,k_13,k_14] = forward(longf,Fxf,Fzf,Myf);
        [k_21,k_22,k_23,k_24] = forward(longf+0.5*h.*[k_11,k_12,k_13,k_14],Fxf,Fzf,Myf);
        [k_31,k_32,k_33,k_34] = forward((longf+0.5*h.*[k_21,k_22,k_23,k_24]),Fxf,Fzf,Myf);
        [k_41,k_42,k_43,k_44] = forward((longf+[k_31,k_32,k_33,k_34].*h),Fxf,Fzf,Myf);
        longf = longf + (1/6).*([k_11,k_12,k_13,k_14]+2.*[k_21,k_22,k_23,k_24]+2.*[k_31,k_32,k_33,k_34]+[k_41,k_42,k_43,k_44]).*h;  % main equation
%         longf = longf + forward(longf,Fxf,Fzf,Myf)*h;

        [Cd1,Cd2,Cd3,Cd4,Cz0,Cz1,Cz2,Cm0,Cm1,Cm2] = param(longf(1),longf(2));
        [Fxf,Fzf,Myf] = FnM(Cd1,Cd2,Cd3,Cd4,Cz0,Cz1,Cz2,Cm0,Cm1,Cm2,u1,u2,longf(1),longf(2));
        
        %backward analysis

        [k_11,k_12,k_13,k_14] = backward(longb,Fxb,Fzb,Myb);
        [k_21,k_22,k_23,k_24] = backward(longb+0.5*h.*[k_11,k_12,k_13,k_14],Fxb,Fzb,Myb);
        [k_31,k_32,k_33,k_34] = backward((longb+0.5*h.*[k_21,k_22,k_23,k_24]),Fxb,Fzb,Myb);
        [k_41,k_42,k_43,k_44] = backward((longb+[k_31,k_32,k_33,k_34].*h),Fxb,Fzb,Myb);
        longb = longb + (1/6).*([k_11,k_12,k_13,k_14]+2.*[k_21,k_22,k_23,k_24]+2.*[k_31,k_32,k_33,k_34]+[k_41,k_42,k_43,k_44]).*h;  % main equation
        
%         longb = longb + backward(longb,Fxb,Fzb,Myb)*h;
        
        [Cd1,Cd2,Cd3,Cd4,Cz0,Cz1,Cz2,Cm0,Cm1,Cm2] = param(longb(1),longb(2));
        [Fxb,Fzb,Myb] = FnM(Cd1,Cd2,Cd3,Cd4,Cz0,Cz1,Cz2,Cm0,Cm1,Cm2,u1,u2,longb(1),longb(2));
        
        R = rand;
        if R <= (1-p_c^(1/N))
            if u1 == 2*0^2*k_r
                u1 = 2*1200^2*k_r;
            elseif u1 == 2*1200^2*k_r
                u1 = 2*0^2*k_r;
            end
        end
        R = rand;
...
function [Vxdot,Vzdot,thetadot,qdot] = forward(long,Fx,Fz,My)
    global m Iy
...
function [Vxdot,Vzdot,thetadot,qdot] = backward(long,Fx,Fz,My)
    global m Iy
    Vxdot = -Fx/m + 9.81*sin(long(3)) + long(4)*long(2);
    Vzdot = -Fz/m - 9.81*cos(long(3)) - long(4)*long(1);
    thetadot = -long(4);
